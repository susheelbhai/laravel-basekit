<li class="relative group/menu-item">
    <details class="group" @if($open) open @endif>
        ...
        {{-- Expanded sidebar: submenu in-flow --}}
        <ul data-slot="sidebar-menu-sub" data-sidebar="menu-sub" class="border-sidebar-border mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-l px-2.5 py-0.5 group-data-[collapsible=icon]:hidden">
            {{ $slot }}
        </ul>

        {{-- Icon-collapsed sidebar: flyout (must stay inside <details> so group-open applies; sidebar-content uses overflow-visible in icon mode) --}}
        <div
