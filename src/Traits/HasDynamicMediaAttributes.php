...
trait HasDynamicMediaAttributes
{
    ...
    /**
     * Resolve media-backed attributes on property access ($model->image).
     *
     * Laravel detects accessors with method_exists($this, 'getXAttribute'). Mutators implemented
     * only via __call are invisible to that check, so getAttribute() must handle these keys here.
     * toArray()/JSON still call getImageAttribute() directly, which __call already satisfies.
     */
    public function getAttribute($key)
    {
        if ($key && in_array($key, $this->mediaAttributes ?? [], true)) {
            return $this->getMediaUrl($key);
        }

        if ($key && str_ends_with((string) $key, '_converted')) {
            $collection = str_replace('_converted', '', (string) $key);
            if (in_array($collection, $this->mediaAttributes ?? [], true)) {
